...
export default function LessonScheduleCard({
  ...
  const hasReschedule = variant === "reschedule" && rescheduleDate && rescheduleStartTime && rescheduleEndTime;
    ...
    <div className="w-full flex flex-col gap-2">
      ...
      <div className="w-full rounded-xl p-1 flex justify-start items-center gap-4 bg-white border border-neutral-50">
        ...
        <div className="w-full flex flex-col items-start">
          <Typography 
            variant="label-2" 
            color="neutral-500"
            className={hasReschedule ? "line-through" : ""}
          >
      ...
      {hasReschedule && (
        <div className="w-full rounded-xl p-1 flex justify-start items-center gap-4 bg-white border border-neutral-50">
          ...
          <div className="rounded-lg w-[66px] h-[66px] aspect-square shrink-0 p-4 flex flex-col items-center justify-center bg-electric-violet-50">
            <Typography variant="label-3" color="deep-royal-indigo-200">
              {rescheduleDate.toLocaleDateString(undefined, monthOptions)}
            </Typography>
          ...
          <div className="w-full flex flex-col items-start">
            <Typography variant="label-2" color="neutral-500">
              {formatScheduleDateTime(rescheduleDate, rescheduleStartTime, rescheduleEndTime)}
            </Typography>
